...
# --- Enhanced CSS with Aurora Animation, Professional UI, and Background Color ---
st.markdown("""
<style>
    ...
    /* --- Base & Background --- */
    .stApp {
        background-color: #0f0c29; /* subtle deep blue background */
...
def filter_movies_from_state():
    ...
    st.session_state.filtered_df = df.copy()  # persist filtered df
    return df

@st.cache_data
def get_top_movies(n=50, sort_by='weighted_rating'): return movies.sort_values(by=sort_by,ascending=False).head(n)
...
def display_movie_cards(movie_titles):
    ...
    for i,title in enumerate(movie_titles):
        with cols[i%5]:
            ...
            st.markdown(f"""<a href="?movie={encoded_title}&prev_view={prev_view}&prev_page={prev_page}" target="_self" style="text-decoration:none;">
...
with st.sidebar:
    ...
    else: st.session_state.filter_years=None
...
if 'movie' in params:
    ...
    st.session_state.current_page=int(params.get('page', st.session_state.current_page))
...
if st.session_state.view=='home':
    ...
    if not movie_df.empty:
        ...
        if c1.button("⬅️ Previous", use_container_width=True, disabled=(page<=1)): st.session_state.current_page-=1
        c2.markdown(f"<div style='text-align:center;margin-top:0.5rem;'>Page {page} of {total_pages}</div>",unsafe_allow_html=True)
        if c3.button("Next ➡️", use_container_width=True, disabled=(page>=total_pages)): st.session_state.current_page+=1

elif st.session_state.view=='filtered_results':
    st.header("Filtered Movie Results")
    filtered_df = st.session_state.get('filtered_df', filter_movies_from_state())
    ...
    if details:
        ...
        with st.container():
            ...
            with col2:
                ...
                if details['runtime']!='N/A': st.markdown(f"**Runtime:** {details['runtime']} minutes")
        ...
        st.markdown("<a href='/?view=home' target='_self' class='nav-btn back-btn'>⬅️ Back to Home</a>",unsafe_allow_html=True)
