...
.stButton > button:hover { background-color:#00f2ea; color:#1a1a2e; box-shadow: 0 0 15px #00f2ea; }
...
.footer { text-align:center; padding:2rem; margin-top:3rem; color:#cccccc; border-top:1px solid #4e54c8; }
</style>
...
for key in ['view', 'selected_movie', 'current_page', 'recommendations', 'selected_for_rec']:
    if key not in st.session_state:
        st.session_state[key] = None
st.session_state.view = st.session_state.view or 'home'
st.session_state.current_page = st.session_state.current_page or 1
...
def display_movie_cards(movie_titles):
    ...
    for i, title in enumerate(movie_titles):
        with cols[i % 5]:
            ...
            st.markdown(f"""
            <a href="?view=details&movie={encoded_title}" target="_self" style="text-decoration:none;">
...
if st.session_state.view == 'home':
    ...
    if st.session_state.view == 'top_movies':
        ...
        movie_df = filter_movies_from_state()
        header = "Filtered Movie Results"

    st.header(header)
    if not movie_df.empty:
        total_pages = math.ceil(len(movie_df)/MOVIES_PER_PAGE)
        page = st.session_state.current_page
        start_idx, end_idx = (page-1)*MOVIES_PER_PAGE, page*MOVIES_PER_PAGE
        paginated_titles = movie_df['title'].iloc[start_idx:end_idx].tolist()
        display_movie_cards(paginated_titles)

        c1,c2,c3 = st.columns([3,1,3])
        if c1.button("⬅️ Previous", disabled=(page<=1)):
            st.experimental_set_query_params(view=st.session_state.view, page=page-1)
        c2.markdown(f"<div style='text-align:center;'>Page {page} of {total_pages}</div>", unsafe_allow_html=True)
        if c3.button("Next ➡️", disabled=(page>=total_pages)):
            st.experimental_set_query_params(view=st.session_state.view, page=page+1)
    else:
    ...
    if details:
        st.markdown(f"<a href='/?view=home' target='_self' class='nav-btn back-btn'>⬅️ Back</a>", unsafe_allow_html=True)
        ...
        with st.container():
            ...
            with col2:
                ...
                st.markdown(f"**Directors:** {', '.join(details['directors']) if details['directors'] else 'N/A'}")
...
# --- Footer ---
st.markdown("<div class='footer'>© 2025 MovieMatch. All Rights Reserved.</div>", unsafe_allow_html=True)
