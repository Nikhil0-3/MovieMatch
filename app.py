...
# --- Enhanced CSS with Aurora Animation, Gradient Background, and Shadows ---
st.markdown("""
<style>
    ...
    /* --- Movie Cards --- */
    .movie-card { background: rgba(255,255,255,0.05); border-radius:15px; padding:1rem; margin:10px 0; transition:all 0.3s ease; border:1px solid transparent; min-height:400px; backdrop-filter: blur(5px); box-shadow: 0 4px 15px rgba(0,0,0,0.3);}
...
if st.session_state.view=='home':
    ...
    movie_title = urllib.parse.unquote_plus(st.session_state.selected_movie)
    details = fetch_movie_details(movie_title)
    if details:
        st.markdown(f"<a href='/?view={st.session_state.previous_view}&page={st.session_state.previous_page}' class='nav-btn back-btn'>⬅️ Back to List</a>", unsafe_allow_html=True)
        st.markdown(f"<div class='details-container'><h2>{details['title']} ({details['release_date']})</h2>", unsafe_allow_html=True)
        ...
        st.markdown(f"**Directors:** {', '.join(details['directors'])}")
        st.markdown(f"**Rating:** {details['vote_average']}")
        st.markdown("</div>", unsafe_allow_html=True)
    else:
        st.warning("Movie details not available.")

st.markdown("<div class='footer'>🎬 MovieMatch &copy; 2025. All rights reserved.</div>", unsafe_allow_html=True)
